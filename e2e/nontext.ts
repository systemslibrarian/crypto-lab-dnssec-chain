 ...
 * separates two tokens on purpose: `--border` is a DECORATIVE divider and is
 * never used as a control edge, while `--control-border` (#7d89b3, measured
 * at 5.6:1 on `--bg` and 4.5:1 on `--surface-3`) is applied to every `.btn`,
 * `.tab-btn`, `select`, text input and `.badge`. But a token is only a
 * declaration, and overrides discard it — `.btn[aria-pressed="true"]`,
 * `.btn-primary` and the selected `.tab-btn` all repaint their border to
 * `--accent-strong` over an `--accent` fill, and each `.badge-*` variant
 * repaints its border to its own semantic ink. This oracle judges each
 * control AS PAINTED, at every driven state, including the pressed and hover
 * recolourings only the drive reaches.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks seven tabpanels, and the expensive part is the
     * NSEC3 tab once a run has completed: two dozen recovered-name chips, the
     * meter, the hash trace and three rows of pressed control buttons — all
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // 1px a coloured state will later occupy so nothing shifts. Nothing on
      // this page uses that idiom today: every `.tab-btn` and `.btn` declares
      // a real `--control-border` on all four sides in every state. The guard
      // stays because counting a transparent spacer as a border is how an
      // unselected control with no fill and no painted edge — identified by
      // its text alone, exactly the case the "is it trying to draw itself as
      // a control?" test below exists to exclude — comes to report 1.00:1.

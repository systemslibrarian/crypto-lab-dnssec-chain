 ...
 *    unknown backdrop. This lab's own stylesheet deliberately has NONE: every
 *    tint that carries meaning (`--ok-tint`, `--bad-tint`, `--warn-tint`,
 *    `--info-tint`, `--accent-tint`, and every `.badge-*`, `.check-*`,
 *    `.link-*`, `.callout-*` and `.status-note-*` built on them) is a resolved
 *    literal hex value, precisely so axe can judge it rather than file it
 *    under `incomplete`. The shared top bar is the exception and it is not
 *    this lab's to change: `--cl-ink` is a `color-mix(in srgb, ...)` and every
 *    `.cl-btn`, `.cl-badge` and brand label is painted with it. A
 *    violations-only gate measures none of that.
 *
 *  - TEXT FADED BY AN ANCESTOR'S `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab avoids `opacity`
 *    for muted text on purpose and lowers the colour's LIGHTNESS instead
 *    (`--text-dim`, `--text-muted`), so today the only declarations left are
 *    `.link-pending { opacity: 1 }` — restated explicitly rather than
 *    inherited, so a future edit that fades an unreached chain link is caught
 *    here — and the `reveal` keyframe's start value, which the reduced-motion
 *    block cancels. The walk measures whatever actually wins rather than
 *    trusting any declaration.
 *
 ...
 *    itself nearly scroller-free on purpose — the long hex, base64 and raw
 *    `dig` runs wrap through `overflow-wrap: anywhere` and `white-space:
 *    pre-wrap` instead of scrolling — so today the only clippers are
 *    `.meter-track`'s `overflow: hidden` corner trim and the `.scroller`
 *    helper's `overflow: auto`. The guard stays because content scrolled past
 *    a clipper is not dimmed or partly drawn; it is absent from the frame, and
 *    asking what colour it sits on has no answer. Its rect is outside every
 *    ancestor's box, so the walk would find nothing behind it and fall through
 *    to WHITE, inventing a failure the day `.scroller` gets its first tenant.
 *
 ...
 *    renders one real SVG — the zone-walk ring, whose `<circle>` nodes and
 *    `<line>` edges sit behind two `<text>` captions — plus the shared
 *    header's two glyph-free `aria-hidden` marks, one of which (the Menu
 *    hamburger) is built from stroke-only `<line>` elements. That is exactly
 *    the shape the
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     *
     * A driven pass here walks a seven-panel document, and the expensive part
     * is the chain tab with its raw-capture disclosure open: 135 lines of
     * `dig` output inside one `<pre>`, plus every link's checklist, its DS
     * digest comparison and its signed-octet runs — all of them siblings
     ...
     * `.scroller { overflow: auto }` helper — this page
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // of the shape: every explanatory `<details>` ships shut, including the
      // raw-capture disclosures that hold the largest text on the page. The
     ...
     *
     * Every `aria-hidden` element on this page is a glyph span rendered by
     * `dom.ts`: the ✓ / ✗ / ⚠ / ● / ○ inside each `.badge`, the same glyph on
     * each `.check` row, the `↓` arrows between chain links, and the numeric
     * prefix on each tab button — plus the shared header's two SVG marks,
     * which carry no text. Each glyph duplicates the words directly beside it,
     * but each is painted in a SEMANTIC ink (`--ok`, `--bad`, `--warn`) on a
     * tinted surface, which is why `scan()` runs the whole `aria-hidden` set
     * through this walk with the exemption lifted rather than arguing any of
     * them is merely decorative.
     *
     * Nothing on this page hides a VALUE. No key tag, digest, recovered name,
     * failure code or verdict is inside an `aria-hidden` subtree — which was
